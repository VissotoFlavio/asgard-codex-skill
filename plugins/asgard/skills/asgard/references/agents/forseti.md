...
- templates and reviews are present, and Hermod's revision-bound terminal evidence covers every required check;
...
PR create/edit authority covers this repair; use the authenticated `gh` CLI once, then re-read the PR and provider-recognized closing-issue relationship. Zero, multiple, closed, cross-repository, or conflicting candidates are `CHANGES_REQUIRED`; never guess, replace, or add several references.
